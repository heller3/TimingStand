...
void pixel_combine_testbeam(TString root_filename, TString pixel_filename)
{
	...
	vector<int> * miss_event_pixel = new vector<int>();

	TBranch * b_miss_event_pixel = pulse->Branch("miss_event_pixel",&miss_event_pixel);
	pulse->SetBranchAddress("miss_event_pixel",&miss_event_pixel,&b_miss_event_pixel);	
    ...
    for(int i=0;i<(int)pixel_table.size();i++ ){
    	...
    	if(pixel_table[i][1] == 16 && pixel_table[i][3] == 0){
    	    miss_event_pixel.push_back(pixel_table[i][0]);   
    	} //missed events
